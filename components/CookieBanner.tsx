    ...
    <AnimatePresence>
      {show && (
        <motion.div
          ...
          className="fixed bottom-8 left-1/2 z-[9999] w-[calc(100vw-32px)] max-w-[600px]"
        >
          <div className={`relative p-6 rounded-[2.5rem] border shadow-[0_20px_50px_rgba(0,0,0,0.5)] flex flex-col md:flex-row items-center gap-6 ${isDark ? 'glass border-white/10 text-white' : 'bg-white border-slate-200 text-slate-900'}`}>
            <button
              onClick={() => setShow(false)}
              className="absolute top-4 right-6 p-2 hover:bg-white/10 rounded-full transition-colors"
            >
            ...
            <div className="flex-grow text-center md:text-left pr-4">
              ...
              <p className="text-xs text-slate-500 leading-relaxed">
                Utilizamos tecnologias para melhorar sua experiência. Ao continuar, você concorda com nossa <a href="/cookies.html" target="_blank" className="text-indigo-500 hover:underline font-bold">Política de Cookies</a> e dados corporativos.
              </p>
            </div>
