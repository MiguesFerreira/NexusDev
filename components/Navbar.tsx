...
import { Zap, Sun, Moon, Briefcase, Home, Headphones } from 'lucide-react';
...
const NavLink: React.FC<{ children: React.ReactNode, onClick?: () => void, href?: string, active?: boolean, isDark: boolean }> = ({ children, onClick, href, active, isDark }) => {
  const content = (
    <div className={`relative text-sm font-semibold transition-colors group flex items-center gap-2 cursor-pointer py-2 ${
      active 
        ? 'text-indigo-500' 
